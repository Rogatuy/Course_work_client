...
import { getAllLikes, getColorReview } from '../../utils/utils';
...
const Card = ({review}) => {
  ...
  return (
    <div className={`${getColorReview(review.grade)} card col-8 col-sm-5 col-md-10 px-0 border-2 rounded-0 my-4 flex-md-row`} id={review._id}>
      ...
      <div className="col">
        ...
        <div className="col-md-12 d-flex flex-column">
          <ul className="list-group list-group-flush list-group-card">
            <li className={`${getColorReview(review.grade)} list-group-item border-0 d-flex flex-row justify-content-center justify-content-md-start`}>
              <p className="card-text">{review.nameOfPiece}</p></li>
            <li className={`${getColorReview(review.grade)} list-group-item border-0 d-flex flex-row list-group-card justify-content-center justify-content-md-start`}>
              <p className="p-2 px-md-0 m-0">Автор: {review.name}</p>
              {likes > 0 &&
              <p className="p-2 m-0">
                {likes} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-heart-fill" viewBox="0 0 16 16">
            ...
            <li className={` ${getColorReview(review.grade)} list-group-item border-0 list-group-card d-flex flex-row justify-content-center justify-content-md-start  text-italic`}><em>{review.tags.join(' ')}</em></li>
          </ul>
        </div> 
